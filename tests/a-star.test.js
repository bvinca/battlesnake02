...
describe('aStar()', () => {
  ...
  it('returns direct path when clear', () => {
    const path = aStar({ x: 0, y: 0 }, { x: 3, y: 0 }, board, []);
    expect(path).toEqual([
      { x: 0, y: 0 },
      { x: 1, y: 0 },
      { x: 2, y: 0 },
  ...
  it('avoids single obstacle', () => {
    ...
    const path = aStar({ x: 0, y: 0 }, { x: 2, y: 0 }, board, obstacles);
  ...
  it('returns undefined if blocked', () => {
    const obstacles = [[{ x: 1, y: 0 }], [{ x: 0, y: 1 }], [{ x: 1, y: 1 }]];
    const path = aStar({ x: 0, y: 0 }, { x: 2, y: 0 }, board, obstacles);
