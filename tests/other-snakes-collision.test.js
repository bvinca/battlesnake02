...
const { checkOtherSnakesCollision } = await import('../src/other-snakes-collision.js');
...
describe('checkOtherSnakesCollision', () => {
  ...
  beforeEach(() => {
    tailCollision.checkTailCollision.mockImplementation((_, __, ___, safe) => safe);
  });
  ...
  it('does not mark any direction unsafe if no threats', () => {
    ...
    const result = checkOtherSnakesCollision(myHead, opponents, myId, [], { ...safe });
  ...
  it('marks unsafe if adjacent to opponent body', () => {
    ...
    const safe = { up: true, down: true, left: true, right: true };
    const result = checkOtherSnakesCollision(myHead, opponents, myId, [], { ...safe });
    expect(result).toEqual({ up: false, down: false, left: false, right: false });
  });
