...
describe('findClosestFood', () => {
  it('returns undefined if no food is provided', () => {
    ...
    expect(findClosestFood({ x: 0, y: 0 }, null)).toBeUndefined();
  });
...
describe('getDirectionToFood', () => {
  it('returns undefined if no food is passed', () => {
    expect(getDirectionToFood({ x: 1, y: 1 }, null)).toBeUndefined();
  });


  it('returns ["left"] if food is to the left', () => {
    expect(getDirectionToFood({ x: 3, y: 1 }, { x: 2, y: 1 })).toEqual(['left']);
  });


  it('returns ["right"] if food is to the right', () => {
    expect(getDirectionToFood({ x: 3, y: 1 }, { x: 5, y: 1 })).toEqual(['right']);
  });
  ...
  it('returns ["down"] if food is below', () => {
    expect(getDirectionToFood({ x: 3, y: 3 }, { x: 3, y: 1 })).toEqual(['down']);
  });


  it('returns both directions if needed (e.g. ["right", "up"])', () => {
    expect(getDirectionToFood({ x: 1, y: 1 }, { x: 2, y: 3 })).toEqual(['right', 'up']);
  });
